...
import com.example.myradio.AppTypography
...
fun MyRadioTheme(
    ...
    val dynamicIsSupported = Build.VERSION.SDK_INT >= Build.VERSION_CODES.S
    val context = LocalContext.current.applicationContext
    val themeController = remember { ThemeController(dynamicIsSupported, PrefsHelperImpl(context)) }

    val currentMode = themeController.currentMode.value   // SYSTEM, LIGHT, DARK
    val currentStyle = themeController.currentStyle.value // DEFAULT, HIGH_CONTRAST, DYNAMIC
...
fun ThemeMode.displayName(): String {
    return when (this) {
...
fun ThemeStyle.displayName(): String {
    return when (this) {
        ThemeStyle.DEFAULT -> "Стандартный контраст"
        ThemeStyle.HIGH_CONTRAST -> "Высокий контраст"
        ThemeStyle.DYNAMIC -> "Динамические цвета (Обои)"
        ThemeStyle.MEDIUM_CONTRAST -> "Средний контраст"
    }
}